...
WORKDIR /opt/reporting-hub-bop-positions-ui
ENV PATH=/opt/reporting-hub-bop-positions-ui/node_modules/.bin:$PATH

COPY package.json /opt/reporting-hub-bop-positions-ui/
COPY yarn.lock /opt/reporting-hub-bop-positions-ui/

RUN yarn --frozen-lockfile

COPY ./ /opt/reporting-hub-bop-positions-ui/
...
COPY --from=builder /opt/reporting-hub-bop-positions-ui/dist/ /usr/share/nginx/html

# The document the platform composes this service's authorization from, at the
# path a deployment names it by
COPY --from=builder /opt/reporting-hub-bop-positions-ui/src/api /opt/app/src/api
...
# TODO: Need to add 8084 to image-scan whitelist
